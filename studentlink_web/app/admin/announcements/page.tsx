"use client"

import { UnifiedAnnouncements } from "@/components/shared/unified-announcements"

export default function AdminAnnouncementsPage() {
  return (
    <>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#1E2A78] flex items-center">
          <svg className="h-8 w-8 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          Announcements Management
        </h1>
        <p className="text-gray-600">Create, publish, and manage announcements with categories, action buttons, and rich content for students and staff.</p>
      </div>
      <UnifiedAnnouncements 
        userRole="admin"
        title="Announcements Management"
        description="Create and manage system-wide announcements with the new standardized format"
      />
    </>
  )
}
