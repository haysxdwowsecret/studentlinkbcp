...
class ConcernController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Auto-assign to department head if available
            $departmentHead = \App\Models\User::where('department_id', $data['department_id'])
                ->where('role', 'department_head')
                ->where('is_active', true)
                ->first();
            
            if ($departmentHead) {
                $data['assigned_to'] = $departmentHead->id;
            }
            ...
            if ($departmentHead) {
                try {
                    \App\Models\Notification::create([
                        ...
                        'message' => "A new {$concern->type} concern has been assigned to your department: {$concern->subject}",
                        'data' => [
                            'concern_id' => $concern->id,
                            'student_name' => $concern->is_anonymous ? 'Anonymous Student' : $user->name,
                            'priority' => $concern->priority,
    ...
    private function createChatRoomForConcern(Concern $concern): void
    {
        try {
            ...
            // Create initial message from department
            \App\Models\ConcernMessage::create([
                'concern_id' => $concern->id,
                'chat_room_id' => $chatRoom->id,
                'author_id' => $concern->approved_by,
                'message' => 'Your concern has been approved and we are now ready to assist you. Please feel free to ask any questions or provide additional information.',
                'type' => 'system',
                'message_type' => 'system',
                'is_internal' => false,
                'delivered_at' => now(),
